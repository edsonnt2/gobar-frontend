...
const RegisterCommandOrTable: React.FC = () => {
  ...
  const handleSubmit = useCallback(
    async (data: RegisterCmdORTableData) => {
      ...
      try {
        ...
        await api.post('commands/products', {
        ...
        addToast({
          ...
          description:
            ...
            Number(productSelected[0].quantity) <= 1
              ? `Produto adicionado na comanda ${data.command}`
              : `Produtos adicionados na comanda ${data.command}`,
        });

        setProductSelected([]);
        formRef.current?.setFieldValue('command', ' ');
        formRef.current?.getFieldRef('command').focus();
      } catch (error) {
        let errorData;
        ...
        if (whichError === 'Command not found at this Business') {
          errorData = { command: 'Comanda não encontrada' };
        } else {
        ...
        if (errorData) {
          ...
          addToast({
            ...
            description:
              'Ocorreu um erro ao tentar cadastrar os produtos na comanda, por favor, tente novamente !',
          });
        }
      } finally {
  ...
  useEffect(() => {
    if (responseModal.response) {
      formRef.current?.setFieldValue('command', responseModal.response);
      inputRef.current?.focus();
  ...
  return (
    <LayoutBusiness pgActive="register-command-table">
      <Container>
        ...
        <ContentSelect>
          <ScroolSelectedColor />
          <ButtonChange isSelected="command">
        ...
        <Form onSubmit={handleSubmit} ref={formRef}>
          ...
          <Button type="button" onClick={hasSubmit}>
            {loadingForm ? 'Carregando...' : 'LANÇAR PRODUTO'}
          </Button>
        </Form>
